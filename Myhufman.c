	...
	{//вставл€ть в нужное место
		...
		*adrFT = (freqTable)realloc(*adrFT, ++(*puniqchrs) * sizeof(symb));
		...
		if (dontBreakSort) {
			...
			//placeToInsert++;//поправка на последнюю проверку, котора€ не была пройдена, но значение placeToInsert уменьшено 
...
void FTDelLastEl(freqTable *adrFT, unsigned short *puniqchrs) {
	//free((*adrFT)[*puniqchrs]); если будешь мен€ть символы на указатель символов
...
#define ifByteTakenWriteToBufCompr(bitsTaken) if (CHAR_BIT == bitsTaken) {\
	...
	for (int i = 0; i < UCHAR_MAX + 1; i++) //тут интеджер а в выводе с.’оть в бинарном файле вывод правильный есть подозрени€ что может пойти не так
	{
...
void main() {
	...
	FILE *f = fopen("“есты\\example_small.bmp", "rb");
